...
const tomorrow = currentDate.clone().add(1, 'days').format('dddd');

// Time
const todayTime = currentDate.format('HH:mm');
...
const CheckoutPage = ({ isDineIn }) => {
    ...
    const handlePlaceOrder = () => {
        ...
        if (orderType !== 'take_away') {
            if (!additionalInformationStates?.streetNumber || !additionalInformationStates?.houseNumber) {
                toast.error(t('Please enter your street and house number'), {
                    position: 'bottom-right',
                })
                return
            }
                ...
                const handleSuccess = (response) => {
                    ...
                    if (response?.data) {
                        if (
                            ...
                            Router.push(url)
                        } else if (paymenMethod?.toLowerCase()?.includes('razor')) {
                            // Razorpay Implementation
                            const options = {
                                key: process.env.NEXT_PUBLIC_RAZORPAY_KEY || global?.razorpay_key || 'rzp_test_ChangeThisToYourKey', // Use key from env or global settings if available
                                amount: (response?.data?.order_amount || totalAmount) * 100, // Amount in paise
                                currency: global?.currency_code || 'INR',
                                name: global?.business_name || 'CarrotFoo Delivery',
                                description: `Order #${response?.data?.order_id}`,
                                image: global?.logo_full_url, // Optional: Add logo url if available
                                order_id: response?.data?.razorpay_order_id, // If backend creates a Razorpay order
                                handler: function (response) {
                                    // Handle success
                                    // Verify payment on backend if needed, or just redirect
                                    // For now, redirecting to success as per user requirement "show in orders page"

                                    // You might want to call a verification API here:
                                    // verifyPayment(response.razorpay_payment_id, response.razorpay_order_id, response.razorpay_signature)

                                    toast.success('Payment Successful!');
                                    setOrderSuccess(true);

                                    // Redirect can be handled by the order success state or manually here if needed
                                    // If setOrderSuccess(true) triggers a redirect/modal, rely on that.
                                    // Otherwise:
                                    Router.push('/order');
                                },
                                prefill: {
                                    name: userOnSuccessHandler?.name || guestUserInfo?.contact_person_name,
                                    email: userOnSuccessHandler?.email || additionalInformationStates?.dine_in_contact?.email,
                                    contact: userOnSuccessHandler?.phone || guestUserInfo?.contact_person_number
                                },
                                notes: {
